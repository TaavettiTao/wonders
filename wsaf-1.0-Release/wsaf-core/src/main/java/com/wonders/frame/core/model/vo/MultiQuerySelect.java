...
public class MultiQuerySelect {
	...
	public String getSqlWithNoAliaNameRepeat(){		
		if(selectFields==null) return "";
		
		StringBuilder sql=new StringBuilder();
		for(MultiQuerySelectField msf:selectFields){
			...
			if(msf.getEntityAlias()!=null&&!msf.getEntityAlias().equals("")){
				sql.append(msf.getEntityAlias()).append(".").append(msf.getColName()).append(" ").append(msf.getEntityAlias()).append("_").append(msf.getFieldName());
			}else{
				sql.append(msf.getColName()).append(" ").append(msf.getFieldName());
			}
	...
	public HashMap<String,MultiQueryField> getSubQueryEntityAttributeMap(){
		...
		for(int i=0;i<selectFields.size();i++){
			MultiQuerySelectField msf=selectFields.get(i);
			//String aliasNum="_"+i;
			hmAttribute.put(msf.getFieldName(),new MultiQueryField(msf.getFieldName(),msf.getFieldType()));
