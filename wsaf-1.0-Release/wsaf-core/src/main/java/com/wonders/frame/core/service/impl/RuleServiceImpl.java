...
public class RuleServiceImpl implements RuleService{
	...
	public List<String> getRulePath(Integer ruleTypeId){
		List<String> rulePaths=RulePathUtil.getRulePath(ruleTypeId);
		if(rulePaths==null){
			List<Rule> rules = findByRuleTypeId(ruleTypeId);

			if (rules != null && rules.size() > 0) {
				
				rulePaths=getRulePath(rules);					
				// 将规则路径存入缓存
				RulePathUtil.setRulePath(ruleTypeId, rulePaths);
			}
	...
	private List<String> getRulePath(List<Rule> rules){
	...
	private List<String[]> getRuleList(List<Rule> rules){
	...
	private List<String> getRuleObjs(List<Rule> rules){
	...
	private  void findAndAddNextRule(String pTypeObj,
...
//				}
	...
	private List<String> convertRulePath2String(List<List<String[]>> rulePathList){
