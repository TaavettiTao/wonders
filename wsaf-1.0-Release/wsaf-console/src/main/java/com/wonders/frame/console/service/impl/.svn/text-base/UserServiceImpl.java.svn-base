...
public class UserServiceImpl implements UserService {
	...
	public ReturnObj<LoginInfo> login(String loginName, String password, String ruleType) {
		...
		return login(loginName, password, rt);
	}
	@Override
	public ReturnObj<LoginInfo> login(String loginName, String password, Integer ruleTypeId) {
		//查找对应规则类型
		RuleType rt=ruleTypeService.findById(ruleTypeId);
		if(rt==null){			
			return new ReturnObj<LoginInfo>("Has no matched ruleType with id:"+ruleTypeId);
		}
		
		return login(loginName, password, rt);
	}
	
	private ReturnObj<LoginInfo> login(String loginName, String password, RuleType rt){
		//根据登陆名查找用户
		...
		if(userRelations==null||userRelations.size()==0){
			return new ReturnObj<LoginInfo>("the user with loginName:"+loginName+" has no related objs in ruleType:"+rt.getName());
		}
		...
		//有多个用户，则返回异常信息
        if(hasDoubleUser){
			return new ReturnObj<LoginInfo>("Find more than one user with loginName:"+loginName+" in ruleType:"+rt.getName());
        }
