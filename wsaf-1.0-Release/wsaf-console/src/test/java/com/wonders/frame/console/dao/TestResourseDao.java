...
public class TestResourseDao extends AbstractTestDao<Resource>{
	...
	public void findByName(){
		Resource rec=dao.findByName(getFirstRec().getName());
		Assert.assertNotNull(rec);
		Assert.assertEquals(getFirstId(),rec.getId());
	}
	
	@Override
	public Resource buildRec(String index) {
		return new Resource("测试name"+index, "测试type"+index, "测试path"+index, "测试description"+index, 0);
	}

	@Override
	public void modifyRec(Resource rec) {
		rec.setName("测试修改");
