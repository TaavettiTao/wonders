...
public class TestGroupDao extends AbstractTestDao<Group>{
	...
	public void findByName(){
		Group rec=dao.findByName(getFirstRec().getName());
		Assert.assertNotNull(rec);
		Assert.assertEquals(getFirstId(),rec.getId());
	}
	
	@Override
	public Group buildRec(String index) {
		return new Group("测试name"+index, "测试type"+index,0);
	}

	@Override
	public void modifyRec(Group rec) {
		rec.setName("测试修改");
