...
<script type="text/javascript">
	    ...
	    function checkForm(){
			if(confirm('是否确认添加？')){
				var value = "";
				value = $("input[name=sendId]").val();	//合同名称
// 				if(value==null || value==""){
// 					$("input[name=sendId]").focus();
// 					alert("请输入发文字号！");
// 					return false;
// 				}
				var _data=$("#form").serialize();
				 $.ajax({
						url: '<%=basePath%>/addRelation/addAjax',
						type : 'POST',
						data:_data,
						async: false,
						dataType: 'text',
...
<%-- 									class="text-input small-input" /></td> --%>
						...
						<tr>
							<td><input name="groupOld.name"
									class="text-input small-input" /></td>
							<td><input name="groupOld.type"
									class="text-input small-input" /></td>
							<td><input name="groupOld.description"
									class="text-input small-input" /></td>
						</tr>
					</tbody>
				</table>
...
<%-- 									class="text-input small-input" /></td> --%>
			...
			<div class="content-box-content">
				...
				<p>
					<br /> <input class="button" type="button" value="提交" onclick="return checkForm();" />
				</p>
			</div>
		</form>
