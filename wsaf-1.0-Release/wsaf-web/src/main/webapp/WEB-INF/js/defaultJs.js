...
//				        		"<a href=\"http://localhost:8080/wsaf-web/jsp/rule/list/"+entry.id+"\" target=\"_blank\" title=\"配置规则\">配置规则</a></td>";
							...
							var tableTr = tableItem+"<td class=\"lvitem\"><div style=\"display:none\" >" +
							"<a href=\"javascript:void(0);\" onclick=\"javascript:$.del('"+tmpModname+"','"+entry.id+"',"+getUrlParam('ruleTypeId')+");\" title=\"删除\"><img src=\"images/icons2/delete_16.png\"/></a>&nbsp;&nbsp;";
						}else{
							var tableTr = tableItem+"<td class=\"lvitem\"><div style=\"display:none\" >" +
...
//					$(modname).find('tr').find("td:eq(" + colnum.toString() + ")").hide(); 
			...
			$(modname+" thead > tr").append(tableTitle+"<th class=\"tableItem\" style=\"width:12%;\">操作</th>");
...
//					opercol: [{ opername: "配置规则",operclass:"ssa",href:basePath+"/jsp/rule/list?ruleTypeId="}], //配置表格的编辑列 ctrid:为关联的dom元素id 
					opercol: [{ opername: "配置规则",operclass:"",onclick:"generRuleTable"},
					          { opername: "绑定",operclass:"ssa",href:basePath+"/jsp/relation/bindTree?ruleTypeId="}], //配置表格的编辑列 ctrid:为关联的dom元素id 
					onok: function (e) { 
						return postData(e);
					}, 
					ondel: function (modname,id) { 
						var ruleTypeId = getUrlParam('ruleTypeId');
						return $.del(modname,id,ruleTypeId); //返回false表示成功，dom元素相应有变化 
					}
				}); 
//				var allRuleType = getAllRuleType();
//				var loginName = $("#loginName");
//				var loginName = 'xiaoxiao1';
//				var ruleTypeArr=[];
//				if(loginName!=null || loginName !=undefined){
//					$.each(allRuleType,function(key,value){
//						var id = value.id;
//						
//						 $.ajax({
//						        type: 'get',
...
//					});
//					
//					$trs = $(modname+" table>tbody tr");
//				       
//				        $trs.each(function (i, tr) {
//				            $tds = $(tr).find("td");
//				            var $id = $($tds[0]).find("input").val();
//				            if($.inArray(parseInt($id),ruleTypeArr)<0){
//				            	$(tr).hide();
//				            }
//				        });
//				}
...
$.getShowForm = function(modname,allTitle,hideField){
	...
	//针对下拉多选调整样式
	$("#"+modname+"FilterForm>div").css("padding-bottom","5px");
	$("#"+modname+"FilterForm>div>a").css("height","19px");
	$("#"+modname+"FilterForm>div>a>span").css("height","15px").css("padding-top","3px");
...
$.postSearchForm = function(firUrl, objname, pageNum, pageSize, queryParams) {
	var modname = objname.substring(1);
	if(modname == 'ruleType'){
	...
	//解决replace只能替换第一个
	var data = $(objname + "form").serialize().replace(/=/g,"_l=");
	if (firUrl != "" && objname != "") {
		var menuUrl = firUrl + "/info";
		$.ajax({
			url : url,
   ...
   $.getJSON(firUrl+objname+"/info", function(data, status, xhr) {
		if (status == "success" && data != null)
		{
			...
			{	
				...
				$("#"+objname+"form>div").css("padding-bottom","5px");
			}
		}
				...
				if (status == "success" && data != null)
				{
					...
					{	
						...
						if(oper == 'add'){
							...
							for ( var i = 0; i < _items.length; i++){
								editInfo.data[0][_items[i].path] = ((editInfo.data[0][_items[i].path]==null) ? "" : editInfo.data[0][_items[i].path]);
								switch(_items[i].operate)
								{
								case "input":
						...
						//针对下拉多选调整样式
						$("#"+objname+"operForm>div").css("padding-bottom","5px");
						$("#"+objname+"operForm>div>a").css("height","19px");
						$("#"+objname+"operForm>div>a>span").css("height","15px").css("padding-top","3px");
					}
				}
			},
			error : function() {
			...
			$.ajax({
				...
				success : function(result) {
					if (result.info.success) {
						...
						var ruleTypeId = getUrlParam("ruleTypeId");
						//同步刷新tree
						onloadZTree_async('treeDemo', ruleTypeId, objname, objname,
								false, true);
						// 重新获取表格-刷新
						$.postSearchForm(basePath + "/" + objname, "#" + objname, 1,
								seleBtns.val(), queryParams);
						alert("操作成功!");
						$("div#"+objname+"OperDialog.ui-dialog-content.ui-widget-content").dialog("close");
					}else{
						var error = "";
						$.each(result.info.error,function(k,v){
							error+=v;
						});
						alert(error);
					}
