...
//		zTree.addNodes(null, newNode);
		...
		var pdiv = $(".content_wrap").attr("id");
		var pmodname = pdiv.substring(0,pdiv.indexOf("Div"));
		$.postOperForm(basePath + '/', pmodname, 0, 'add');
		return false;
	});
}
...
//	if(treeId!="treeReverseDemo"){
//		addStr += "<span class='button add' id='addBtn_" + treeNode.tId
//		+ "' title='add nodes' onfocus='this.blur();'></span>";
//	}
...
//			zTree.selectNode(treeNode);
			...
			if (reverseTree) {
				var pdiv = $(".content_wrap").attr("id");
				var pmodname = pdiv.substring(0,pdiv.indexOf("Div"));
				$.postOperForm(basePath + '/', pmodname, id, 'add');
			
//				onloadZTree_reverse_async(treeId, treeNode.ruleTypeId, pmodname,
//						pmodname, true, true);
			} else {
//				var zTree = $.fn.zTree.getZTreeObj(treeId);
...
//				onloadZTree_async(treeId, treeNode.ruleTypeId, pmodname,
//						pmodname, true, true);
			}
			return false;
		});
...
function getTree(ruleTypeId, pmodname, rule) {
	...
	if($("#"+pmodname+"OperDialog").length==0){
		_strTable += "<div id=\"" + pmodname
		+ "OperDialog\" style=\"display:none\"><form id=\"" + pmodname
		+ "operForm\"></form></div>";
	}
							...
							buttons : {
								"查询" : function() {
									var seleBtns = $("#" + modname).find(
											"[name='number']");
									$.postSearchForm(basePath +"/"+ modname, "#" + modname,
...
//			$.postOperForm(getBasePath() + "/", modname,'add');
//	});
...
function getRuleNodes(onlyNext,rule,pmodname){
	var ruleNodes;
	var tmp = "";
	if(onlyNext){
		for ( var a in rule) {
